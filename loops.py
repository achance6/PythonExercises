...
def letter_grades(highest: int) -> list[int]:
    ...
    """
    thresholds = []
    divide = (highest - 40 - 1) / 4
    threshold = highest - divide
    while len(thresholds) != 4:
        thresholds.insert(0, math.ceil(threshold))
        threshold -= divide
    return thresholds
...
def student_ranking(student_scores: list[int], student_names: list[str]) -> list[str]:
    ...
    for index, score in enumerate(student_scores):
        student = f'{index + 1}. {student_names[index]}: {score}'
        student_ranks.append(student)
